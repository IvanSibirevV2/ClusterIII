...
{
    public static class LoaderLLS_From_Json
    {
        public static void Test_LoaderLLS_From_Json()
        {
            (new Component.Consoller_Shabloner(ConsoleColor.Cyan, ConsoleColor.DarkRed)).WriteLine((new Component.StackTracer()).Get_STSS());
            LoaderLLS_From_Json_WinForm().writeThis(5);
        }
        public static List<List<string>> LoaderLLS_From_Json_WinForm()
        {
            ...
            while(_flagan_){
                ...
                if (!_flagan) foreach (var _fileName in _openFileDialog.FileNames) if (System.IO.File.Exists(_fileName))
                        {
                            _LLS_RESALT = (new LLS_Json_SaveLoadEr()).LoadFromFile(_fileName).p_LLS;
